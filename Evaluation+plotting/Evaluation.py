...
# =======================
NUM_LASER_POINTS = 500  # Anzahl der Punkte auf der gefitteten Laserlinie (Calibration)
NUM_DENSE_SAMPLES = 1000  # Anzahl der dicht gesampelten Punkte auf der Polyline (Measurement)

# =======================
# AUSREISSER-FILTERUNG
# =======================
OUTLIER_MEDIAN_WINDOW = 15       # Halbe Fensterbreite für lokalen Median (in Anzahl Nachbarn)
OUTLIER_MAD_FACTOR = 3.5         # Ausreißer-Schwelle: Punkt gilt als Ausreißer wenn |y - median| > Faktor * MAD
OUTLIER_MAD_FLOOR = 5.0          # Mindest-MAD in Pixel (verhindert Überfilterung in glatten Bereichen)
SLOPE_THRESHOLD = 0.5            # (DEPRECATED - wird ignoriert, für Kompatibilität beibehalten)
...
def create_polyline_with_outlier_filter(reddest_pixels, floor_size, slope_threshold=5.0,
    ...
    1. Sortiert die Pixel nach X-Koordinate
    2. Erkennt Ausreißer anhand von lokalem Median + MAD (Median Absolute Deviation)
    3. Ersetzt Ausreißer durch lokalen Median-Wert
    ...
    Args:
        reddest_pixels: Liste von (x, y) Pixel-Koordinaten
        floor_size: Größe des Boden-ROI
        slope_threshold: (DEPRECATED - wird ignoriert) Für Kompatibilität beibehalten
        num_dense_samples: Anzahl der dicht gesampelten Punkte
    ...
    print(
        f"[INFO] Polyline mit Ausreißer-Filterung (Rolling Median + MAD): {n} Punkte, X-Bereich: {x_vals.min():.1f} bis {x_vals.max():.1f}")
    print(f"       Median-Fenster: ±{OUTLIER_MEDIAN_WINDOW} Nachbarn")
    print(f"       MAD-Faktor: {OUTLIER_MAD_FACTOR}, MAD-Floor: {OUTLIER_MAD_FLOOR} px")
    print(f"       Fit-Methode: {fit_method}")

    # Schritt 2: Ausreißer-Erkennung mit Rolling Median + MAD
    outlier_mask = np.zeros(n, dtype=bool)
    local_medians = np.zeros(n, dtype=float)
    local_mads = np.zeros(n, dtype=float)
    deviations = np.zeros(n, dtype=float)

    for i in range(n):
        # Definiere Fenster um Punkt i
        window_start = max(0, i - OUTLIER_MEDIAN_WINDOW)
        window_end = min(n, i + OUTLIER_MEDIAN_WINDOW + 1)

        window_y = y_vals[window_start:window_end]

        # Berechne lokalen Median
        local_median = np.median(window_y)
        local_medians[i] = local_median

        # Berechne lokale MAD
        abs_deviations = np.abs(window_y - local_median)
        local_mad = np.median(abs_deviations)
        local_mads[i] = local_mad

        # Berechne Abweichung des aktuellen Punktes vom lokalen Median
        deviation = abs(y_vals[i] - local_median)
        deviations[i] = deviation

        # Markiere als Ausreißer wenn Abweichung > MAD_FACTOR * max(MAD, MAD_FLOOR)
        mad_threshold = max(local_mad, OUTLIER_MAD_FLOOR)
        if deviation > OUTLIER_MAD_FACTOR * mad_threshold:
            outlier_mask[i] = True

    num_outliers = np.sum(outlier_mask)
    outlier_percent = (num_outliers / n) * 100

    print(f"[INFO] Ausreißer erkannt: {num_outliers} von {n} Punkten ({outlier_percent:.1f}%)")

    # Debug-Ausgaben für erste 5 Ausreißer
    if num_outliers > 0:
        outlier_indices = np.where(outlier_mask)[0]
        print(f"[INFO] Erste {min(5, num_outliers)} Ausreißer:")
        for idx in outlier_indices[:5]:
            print(f"       X={x_vals[idx]:.1f}: y_orig={y_vals[idx]:.1f}, y_median={local_medians[idx]:.1f}, "
                  f"Abweichung={deviations[idx]:.1f}, MAD={local_mads[idx]:.1f}")

    # Globale Statistiken
    print(f"[INFO] MAD-Statistiken:")
    print(f"       Mittlere MAD: {local_mads.mean():.2f} px")
    print(f"       Max MAD: {local_mads.max():.2f} px")
    print(f"       Max Abweichung: {deviations.max():.2f} px")

    # Schritt 3: Ausreißer ersetzen durch lokalen Median
    if num_outliers > 0:
        outlier_indices = np.where(outlier_mask)[0]
        for idx in outlier_indices:
            y_vals[idx] = local_medians[idx]
...
def compare_calibration_measurement_profiles(calibration_data, measurement_data, output_path):
    """
    Vergleicht Calibration- und Measurement-Profile OHNE Plotting.
    Speichert nur X/Y/Z-Koordinaten aller Messpunkte in CSV.
    ...
    """
    print(f"\n{'=' * 60}")
    print("PROFIL-VERGLEICH (NUR CSV-EXPORT)")
    print(f"{'=' * 60}\n")
    ...
    meas_reddest_pixels = measurement_data['reddest_pixels']
    ...
    # 3. Berechne inverse Transformation
    ...
    if calibration_result:
        ...
        if measurement_result:
            ...
            comparison_result = compare_calibration_measurement_profiles(
                calibration_data=calibration_result,  # ← RICHTIG: Kalibrierungsdaten
                measurement_data=measurement_result,  # ← RICHTIG: Messdaten
                output_path=calibration_result['output_dir'] / "profile_comparison.png"  # ← RICHTIG: Output-Pfad
            )
